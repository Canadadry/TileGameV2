...
namespace TMX
{
	...
	private:
		...
		TileSet* readTileSet(TiXmlNode* node)
		{
			...
			PRINT("name %s\n",tileset->name.c_str());
			pElement->QueryIntAttribute("firstgid",&(tileset->firstGlobalID));
		...
		Layer* readLayer(TiXmlNode* node)
		{
			PRINT("Reading Layer\n");
		...
		void decodeblock( unsigned char* in, unsigned char* out ) //in must contain 4 char, out must contain 3 char
		{
	...
	std::auto_ptr<Map> Loader::LoadFromFile(const std::string& filename)
	{
		PRINT("Start reading file : %s\n",filename.c_str());
		ImplementedLoader private_loader(filename);
		return std::auto_ptr<Map>(private_loader.m_map);
	}
